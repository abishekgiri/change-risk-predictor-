...
def perform_analysis(repo_slug, pr_id, config, token=None):
    ...
    # TRANSFORM: Map Risk Terms to Compliance Terms
    # This acts as the adapter layer until we fully refactor RiskScorer
    decision_map = {"FAIL": "BLOCK", "WARN": "WARN", "PASS": "COMPLIANT"}
    result["control_result"] = decision_map.get(result.get("decision"), "COMPLIANT")
    result["severity"] = result.get("risk_score", 0)
    # Remove old keys optionally, or keep for compat
