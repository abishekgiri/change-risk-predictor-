...
class ApprovalsControl(ControlBase):
 ...
 review = Review(
 reviewer=pr_review.get("user", {}).get("login", "unknown"),
 state=pr_review.get("state", "COMMENTED"),
 submitted_at=datetime.fromisoformat(
 pr_review.get("submitted_at", "").replace("Z", "+00:00")
 ),
 commit_id=pr_review.get("commit_id", "")
 )
 reviews.append(review)
 
 return reviews
 except Exception as e:
 # If fetching fails, return empty (will fail validation if approvals required)
 return []
